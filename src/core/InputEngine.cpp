...
$Id: InputEngine.cpp,v 1.18 2007/01/18 11:33:38 r_sijrier Exp $
*/
...
int InputEngine::broadcast_action(IEAction* action)
{
	...
	if (k && (!isHolding)) {
		if (k->prepare_actions()) {
			...
			// If the fact was generated by a wheelevent,
			// then we have to accept the event here!
			if (m_wheelEvent) {
				printf("accepting wheelevent\n");
				m_wheelEvent->accept();
			}
		} else {
...
int InputEngine::identify_first_fact()
{
	...
	// Wheel events can only be of type < K > 
	// Now if m_wheelEvent exists, we have to ignore the event
