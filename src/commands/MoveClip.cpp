...
int MoveClip::do_action()
{
        ...
        if (!targetTrack) {
                ...
                originTrack->remove_clip(m_clip);
                m_clip->set_track_first_block(newInsertBlock);
...
int MoveClip::undo_action()
{
        ...
        m_clip->set_track_first_block(originalTrackFirstBlock);
...
int MoveClip::jog()
{
        ...
        nframes_t origTrackStartFrame = m_clip->get_track_first_block();
