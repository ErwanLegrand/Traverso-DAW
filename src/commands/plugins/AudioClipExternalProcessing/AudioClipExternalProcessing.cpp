...
Command* AudioClipExternalProcessing::create(QObject* object)
{
	AudioClip* clip = qobject_cast<AudioClip*>(object);
	
	if (!clip) {
		PERROR("Object object is NOT an AudioClip but of type %s!!\n", object->metaObject()->className());
		return 0;
	}

	return new Processor(clip);
}

QString AudioClipExternalProcessing::name() const
{
	return metaObject()->className();
}


Q_EXPORT_PLUGIN2(tc_audioclipexternalprocessing, AudioClipExternalProcessing)


Processor::Processor(AudioClip* clip)
	: Command(clip, QObject::tr("Clip: External Processing"))
{
	m_clip = clip;
...
int Processor::prepare_actions()
{
	...
	ReadSource* rs = manager->get_readsource(m_clip->get_readsource_id());
