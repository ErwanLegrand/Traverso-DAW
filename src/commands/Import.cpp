/*
    ...
    $Id: Import.cpp,v 1.1 2006/04/20 14:51:13 r_sijrier Exp $
*/
...
int Import::prepare_actions()
{
        ...
        for (int channel=0; channel < channels; channel++) {
                if ( (existingSource = project->get_source( m_fileName, channel)) != 0) {
                        PWARN("Using existing AudioSource object");
                        m_clip->add_audio_source((ReadSource*)existingSource, channel);
                } else {
                        PWARN("Creating new AudioSource object");
                        ReadSource* newSource;
                        newSource = new ReadSource(channel, dir, name);

                        if (newSource->init() < 0) {
                                // Hmmmm this will mess up certain things a little but is unlikey to happen....
                                PERROR("Failed to initialize AudioSource %s for channel %d", m_fileName.toAscii().data(), channel);
                                return -1;
                        }
                        ...
                        project->add_audio_source(newSource, channel);
